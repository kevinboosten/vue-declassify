...
export function extract(source: SourceFile) {
  ...
  let declaration = defaultExport.getValueDeclaration();
...
export function removeAccessModifiers(declaration: ClassDeclaration): ClassDeclaration {
  for (const member of declaration.getInstanceMembers()) {
    member.toggleModifier("public", false);
    member.toggleModifier("protected", false);
